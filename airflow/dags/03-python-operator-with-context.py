...
# 앞선 02-python-operator.py는 "date_now = datetimenow().date()"를 사용하기 떄문에
...
# 따라서 코드 상에서 now(), SQL 상에서 current_date() 등을 사용하지 않고, Airflow에서 실행하기로 했던 시간을 넣어줘야 한다.
# execution_date라고 부른다.
...
def print_current_date_with_context_variable(*args, **kwargs):
    """
    {'conf': airflow.configuration.AirflowConfigParser object at 0x1069a8b00>,
    'dag': <DAG: python_dag_with_context>,
    'dag_run': <DagRun python_dag_with_context @ 2022-04-20 00:30:00+00:00:scheduled___2022-04-20T00:30:00+00:00,,
    externally triggered: False>,
    'data_interval_end': DateTime(2022, 4, 21, 0, 30, 0, tzingfo=Timezone('UTC')),
    'data_interval_start': DateTime(2022, 4, 20, 0, 30, 0, tzinfo=Timezone('UTC')),
    'ds': '2022-04-20',
    'ds_nodash': '20220420',
    'next_execution_date': <Proxy at 0x108658248 with factory <function TaskInstance.get_template_context.<locals>.deprecated_proxy.<locals>.deprecated_func at 0x108654ae8>>,
    }   
    """
...
with DAG(
    ...
    PythonOperator(
        ...
        provide_context=True # True일 경우 Airflow Task Instance의 Attribute를 Keyword Argument로 Python 함수에서 사용할 수 있음
    )