...
def train_start(**context):
    ...
    tmp = (
        requests.post(
            f"http://118.67.132.123:30001/modeling",
            json={"name": context["params"]["name"], "config": {}},
        ).json(),
    )
...
with DAG(
    ...
    task_confirm_data = BranchPythonOperator(
        task_id="confirm_data", python_callable=confirm_data
    )
    ...
    task_not_change = PythonOperator(task_id="not_change", python_callable=not_change)
